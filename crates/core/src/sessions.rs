...
/// The prune parts a caller may request (mirrors `session.PRUNE_PARTS`).
pub const PRUNE_PARTS: [&str; 3] = ["screenshots", "screenshots_halve", "audio"];

/// The capability flags for a session dir, as a JSON object (`has_screenshots`/`has_audio`/`has_mic`/
/// `can_retranscribe`/`can_index`) — recomputed from the current on-disk artifacts. The daemon merges
/// these into a prune response + the session's `session.json` summary.
pub fn capabilities(dir: &Path) -> serde_json::Value {
    let c = session_capabilities(dir);
    serde_json::json!({
        "has_screenshots": c.has_screenshots,
        "has_audio": c.has_audio,
        "has_mic": c.has_mic,
        "can_retranscribe": c.can_retranscribe,
        "can_index": c.can_index,
    })
}

/// Apply prune `parts` to a finished session's dir; returns `(freed_bytes, remaining_screenshot_count)`.
/// `screenshots` deletes all frames, `screenshots_halve` drops every other (keeping timeline coverage
/// at half the cadence), `audio` removes `audio.s16le`/`mic.s16le`. Mirrors `session.prune_session_dir`.
pub fn prune_session_dir(dir: &Path, parts: &[String]) -> (u64, usize) {
    let shots = dir.join("screenshots");
    let has = |p: &str| parts.iter().any(|x| x == p);
    let sz = |f: &Path| std::fs::metadata(f).map(|m| m.len()).unwrap_or(0);
    let mut freed = 0u64;

    if has("screenshots") && shots.is_dir() {
        if let Ok(it) = std::fs::read_dir(&shots) {
            for e in it.flatten() {
                if e.path().is_file() {
                    freed += sz(&e.path());
                }
    ...
    } else if has("screenshots_halve") && shots.is_dir() {
        let mut files: Vec<PathBuf> = std::fs::read_dir(&shots)
            .map(|it| it.flatten().map(|e| e.path()).filter(|p| p.is_file()).collect())
            .unwrap_or_default();
        files.sort();
        // Keep 0,2,4… → delete 1,3,5…
    ...
    let count = std::fs::read_dir(&shots)
        .map(|it| it.flatten().filter(|e| e.path().is_file()).count())
        .unwrap_or(0);
    (freed, count)
}

/// Remove a session id from the append-only index (`sessions.jsonl`), rewritten atomically. The
/// caller removes the on-disk dir separately. Mirrors `registry._rewrite_index_locked`.
pub fn remove_from_index(session_id: &str) {
    ...
    for ln in text.lines() {
        ...
        let drop = serde_json::from_str::<serde_json::Value>(ln)
            .ok()
            .and_then(|v| v.get("session_id").and_then(|s| s.as_str()).map(|s| s == session_id))
            .unwrap_or(false);
        if !drop {
            kept.push_str(ln);
            kept.push('\n');
...
/// Merge `updates` into a session's `session.json` `summary` object (e.g. a new screenshot count +
/// capability flags after a prune, or `transcript_segments` after a re-transcribe). Mirrors
/// `registry._rewrite_session_json_summary`.
pub fn rewrite_session_summary(dir: &Path, updates: &serde_json::Value) {
    ...
    if let (Some(summary), Some(upd)) =
        (v.get_mut("summary").and_then(|s| s.as_object_mut()), updates.as_object())
    {
        for (k, val) in upd {
...
/// A bool field from the recorded summary block (None when absent or not a bool).
fn s_bool(summary: &Value, key: &str) -> Option<bool> {
    summary.get(key).and_then(|v| v.as_bool())
}

/// The recorded `notes` array as `Vec<String>` (empty when absent or not an array of strings).
fn s_notes(summary: &Value) -> Vec<String> {
    summary
        .get("notes")
...
/// The note `_recover` appends when a live-state session is recovered after its process exited.
const INTERRUPTED_NOTE: &str =
    "recovered from disk; the capturing process exited while this session was live";

/// The note `_recover` sets when a session.json has no usable `summary` block.
const MISSING_SUMMARY_NOTE: &str = "recovered from index; session.json missing or unreadable";
...
pub fn recover_session(session_dir: &Path) -> Option<v1::Session> {
    ...
    // notes: the recorded list, with the interrupted note appended when we rewrote the state
    // (mirrors `_recover`'s `notes + [...]`).
    let mut notes = s_notes(&summary);
    if interrupted {
        notes.push(INTERRUPTED_NOTE.into());
    }
    ...
    // Everything else is `{**template, **recorded}`: the recorded value when present, else the
    // template default (`registry._template`).
    Some(v1::Session {
        session_id,
        state,
        dir,
        pid: s_i64(&summary, "pid"),
        window_title: s_str(&summary, "window_title"),
        started_at: s_str(&summary, "started_at"),
        stopped_at: s_str(&summary, "stopped_at"),
        screenshots,
        screenshot_errors: s_i64(&summary, "screenshot_errors").unwrap_or(0),
        log_lines: s_i64(&summary, "log_lines").unwrap_or(0),
        process_running: s_bool(&summary, "process_running"),
        audio_mode: s_str(&summary, "audio_mode").unwrap_or_else(|| "unknown".into()),
        audio_status: s_str(&summary, "audio_status").unwrap_or_else(|| "unknown".into()),
        transcript_segments,
        asr_errors: s_i64(&summary, "asr_errors").unwrap_or(0),
        mic_status: s_str(&summary, "mic_status").unwrap_or_else(|| "off".into()),
        mic_segments: s_i64(&summary, "mic_segments").unwrap_or(0),
        mic_device: s_str(&summary, "mic_device"),
        capture_preset: s_str(&summary, "capture_preset"),
        index_preset: s_str(&summary, "index_preset"),
...
fn template_session(session_id: String, dir: String, caps: &Caps) -> v1::Session {
    v1::Session {
        ...
        can_retranscribe: caps.can_retranscribe,
        can_index: caps.can_index,
        notes: vec![MISSING_SUMMARY_NOTE.into()],
    }
}
...
/// Resolve a single session by id (the read-side of `registry.summary(sid)`): scan `runs`, fold
/// the index, and return the recovered [`v1::Session`] whose `session_id` matches — or `None` when
/// no such session exists. Uses the same enumeration as [`list_sessions`] so a session reachable
/// only through the index (e.g. an out-of-runs import) still resolves.
pub fn find_session(runs: &Path, id: &str) -> Option<v1::Session> {
    list_sessions(runs).into_iter().find(|s| s.session_id == id)
}
...
mod tests {
    ...
    fn recover_complete_session() {
        ...
        // Expanded SessionSummary fields ride through from the recorded summary.
        assert_eq!(s.started_at.as_deref(), Some("2026-06-16T22:01:13.146Z"));
        assert_eq!(s.stopped_at.as_deref(), Some("2026-06-16T22:05:00.000Z"));
        assert!(s.notes.is_empty(), "a clean stop carries no recovery note");
    ...
    fn recover_interrupted_session() {
        ...
        assert!(
            s.notes.iter().any(|n| n.contains("the capturing process exited")),
            "the interrupted recovery note is appended"
        );
        fs::remove_dir_all(&runs).ok();
    }
    ...
    fn recover_missing_summary_is_unknown() {
        ...
        assert_eq!(s.audio_mode, "unknown");
        assert!(
            s.notes.iter().any(|n| n.contains("session.json missing or unreadable")),
            "the missing-summary recovery note is set"
        );
        fs::remove_dir_all(&runs).ok();
    }
    ...
    #[test]
    fn prune_halves_deletes_and_drops_audio() {
        let nanos = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let runs = std::env::temp_dir().join(format!("capture-prune-{nanos}"));
        let dir = write_session(&runs, "20260616-prune", r#"{"state":"stopped"}"#, 4, true, 0);

        // halve: 4 frames → 2 (drops the 2nd + 4th)
        let (freed, count) = prune_session_dir(&dir, &["screenshots_halve".into()]);
        assert!(freed > 0);
        assert_eq!(count, 2);

        // audio: removes audio.s16le; capabilities reflect it
        prune_session_dir(&dir, &["audio".into()]);
        assert!(!dir.join("audio.s16le").exists());
        let caps = capabilities(&dir);
        assert_eq!(caps["has_audio"], serde_json::json!(false));
        assert_eq!(caps["can_retranscribe"], serde_json::json!(false));
        assert_eq!(caps["has_screenshots"], serde_json::json!(true)); // 2 frames remain

        // rewrite_session_summary merges into session.json's summary
        rewrite_session_summary(&dir, &serde_json::json!({ "screenshots": count }));
        let v: serde_json::Value =
            serde_json::from_str(&fs::read_to_string(dir.join("session.json")).unwrap()).unwrap();
        assert_eq!(v["summary"]["screenshots"], serde_json::json!(2));

        // screenshots: delete all
        let (_f, count0) = prune_session_dir(&dir, &["screenshots".into()]);
        assert_eq!(count0, 0);
        assert_eq!(capabilities(&dir)["has_screenshots"], serde_json::json!(false));
