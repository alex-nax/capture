...
impl CaptureApp {
    ...
    /// Copy the bundled `capture-mcp` path to the clipboard (#78) so the user can paste it as the
    /// `command` in an MCP client's `.mcp.json` — app users get the MCP entry point with no build.
    pub(crate) fn copy_mcp_command(&mut self, cx: &mut Context<Self>) {
        match crate::daemon::bundled_mcp() {
            Some(p) => {
                cx.write_to_clipboard(gpui::ClipboardItem::new_string(p.to_string_lossy().into_owned()));
                self.message = "copied capture-mcp path — paste it as the command in your editor's .mcp.json".into();
            }
            None => {
                self.message = "couldn't find the bundled capture-mcp next to the app".into();
            }
        }
        cx.notify();
    }
    ...
    pub(crate) fn download_model(&mut self, repo: String, cx: &mut Context<Self>) {
        ...
        // Optimistically show a 0% bar so the row reacts immediately, and clear any prior
        // failure on this repo (Retry routes through here) so the row leaves the failed state.
        {
            let mut live = self.live.lock().unwrap();
            live.asr_progress.insert(repo.clone(), 0.0);
            live.asr_failed.remove(&repo);
        }
        ...
        cx.spawn(async move |this, cx| {
            ...
            let _ = this.update(cx, |v, cx| {
                if let Err(e) = r {
                    ...
                    live.asr_progress.remove(&repo);
                    live.asr_failed.insert(repo.clone(), e.clone());
