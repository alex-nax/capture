...
pub(crate) struct LiveState {
    ...
    pub(crate) asr_dl: HashMap<String, DlStat>, // repo -> live bytes + smoothed speed during a model download (#83)
    pub(crate) asr_failed: HashMap<String, String>, // repo -> last download error, kept until retried (#83)
...
/// Live bytes + smoothed transfer rate for an in-flight model download (#83). The SSE
/// `asr_download` event carries real `downloaded`/`total` byte counts; we keep the last
/// sample's time + bytes to derive an EMA-smoothed speed, which feeds the size/%/ETA label.
pub(crate) struct DlStat {
    pub(crate) done: u64,
    pub(crate) total: u64,
    pub(crate) speed_bps: f64,             // exponentially-smoothed bytes/sec (0 until two samples)
    pub(crate) last_t: std::time::Instant, // when `last_done` was sampled
