...
pub struct CaptureApp {
    ...
    pub(crate) asr_cta_dismissed: bool,               // "Not now" on the transcription-setup CTA (#83) — show the minimised pill
    pub(crate) asr_engine_expanded: bool,             // Settings → Voice: expand the engine picker even when a runtime is active (#83 "Change")
    pub(crate) asr_engine_selected: Option<String>,   // Settings → Voice: which engine's config Step 2 shows (the user's pick; else the resolved-active engine) (#83)
...
impl CaptureApp {
    pub fn new(cx: &mut Context<Self>) -> Self {
        ...
        if !under_agent {
            // If no daemon is running, start the Rust `captured` detached — the bundled one in
            // the packaged .app, or (dev) the one built beside this binary in the workspace
            // target. The poll loop picks it up within ~1-2s.
            let running = daemon::discover().map_or(false, |d| d.available());
            if !running {
                if let Some(bin) = daemon::resolve_daemon() {
        ...
        let mut app = Self {
            ...
            jpeg_quality,
            capture_screenshots,
            asr_cta_dismissed: false,
            asr_engine_expanded: false,
            asr_engine_selected: None,
            update_info: None,
            updating: false,
                    ...
                    match ev_type {
                        Some("asr_download") => {
                            if let (Some(repo), Some(frac)) = (
                                ...
                                st.asr_failed.remove(repo); // a fresh byte = no longer failed
                                // The event carries real byte counts; track them + a smoothed
                                // rate so the model row shows `656 MB / 1.6 GB · 41% · ~2 min · 9.4 MB/s`.
                                let done = ev.get("downloaded").and_then(|v| v.as_u64()).unwrap_or(0);
                                let total = ev.get("total").and_then(|v| v.as_u64()).unwrap_or(0);
                                let now = std::time::Instant::now();
                                let e = st.asr_dl.entry(repo.to_string()).or_insert(crate::state::DlStat {
                                    done,
                                    total,
                                    speed_bps: 0.0,
                                    last_t: now,
                                ...
                                if dt >= 0.3 && done >= e.last_done {
                                    let inst = (done - e.last_done) as f64 / dt;
                                    e.speed_bps = if e.speed_bps <= 0.0 { inst } else { e.speed_bps * 0.7 + inst * 0.3 };
                                    e.last_t = now;
                                    e.last_done = done;
                                }
                                e.done = done;
                        ...
                        Some("asr_download_done") => {
                            if let Some(repo) = ev.get("repo").and_then(|v| v.as_str()) {
                                st.asr_progress.remove(repo);
                                st.asr_dl.remove(repo);
                                st.asr_failed.remove(repo);
                            }
                            continue;
                        }
                        Some("asr_download_error") => {
                            if let Some(repo) = ev.get("repo").and_then(|v| v.as_str()) {
                                st.asr_progress.remove(repo);
                                st.asr_dl.remove(repo);
                                let msg = ev.get("error").and_then(|v| v.as_str()).unwrap_or("download failed");
                                st.asr_failed.insert(repo.to_string(), msg.to_string());
                            }
                            continue;
                        }
                ...
                .spawn(async move {
                    match daemon::discover() {
                        Some(d) if d.available() => {
                            ...
                            // Poll the runtime registry too (#83): an engine-pack install/select must
                            // flip the row to "Active" — without this the row stays stale on "Download".
                            let rt = d.asr_runtimes().unwrap_or_default();
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .items_start()
                            .justify_between()
                            // Left: title + daemon line + a status row (hotkey hint · reachable pill · message).
                            .child(
                                div()
                                    .flex()
                                    .flex_col()
                                    ...
                                    .child(
                                        div()
                                            ...
                                            .flex_wrap()
                                            .gap(px(14.0))
                                            .when(!hotkey_hint.is_empty(), |d| {
                                                d.child(
                                                    div()
                                                        .text_size(px(theme::TS_SMALL))
                                                        .text_color(rgb(theme::ACCENT_TEXT))
                                                        .child(hotkey_hint.clone()),
                                                )
                                            })
                                            .when(self.index_status.available, |d| {
                                                // green "endpoint reachable" pill with a leading dot.
                                                d.child(
                                                    div()
                                                        .flex()
                                                        .items_center()
