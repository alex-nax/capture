...
| **GUI v3 redesign** (PLANNED) | [gui-redesign.md](gui-redesign.md) | **Decompose `app.rs` (4.1k lines) by screen/component/domain** (theme/components/screens/domain modules; `pub(crate)` model) **+ apply the locked dark design system** (indigo accent, pure-gray ramp, tight radii) from `design/CAPTURE-HANDOFF.md` + the HTML export. Settings left-nav, runtime-driven Voice recognition, permission states, live playback. Features #68–#76 |
